...
public class Main
{
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println("setSize fm filename length");
        ...
        while (true)
        {
            try
            {
                ...
                else if (command.startsWith("setSize") && command.split(" ").length == 4)
                {
                    setSize(strArr[1], strArr[2], Long.parseLong(strArr[3]));
                }
    ...
    {
        File file = new File("src/FM1");
        if (!file.exists())
        {
            FileManagerImpl fileManager1 = new FileManagerImpl("FM1");
            FileManagerImpl fileManager2 = new FileManagerImpl("FM2");
            FileManagerImpl fileManager3 = new FileManagerImpl("FM3");
            BlockManagerImpl blockManager1 = new BlockManagerImpl("BM1");
            BlockManagerImpl blockManager2 = new BlockManagerImpl("BM2");
            BlockManagerImpl blockManager3 = new BlockManagerImpl("BM3");
            writeBM(3);
        }
        else
        {
            System.out.println("文件管理器已存在，不必初始化");
        }
    }

    public static void setSize(String fm, String filename, long length)
    {
...
//        System.out.println("move后，读数据之前光标位置 "+file.getCursor());
        ...
        //读完后将光标置尾
        file.setCursor(file.getFileSize() );
    ...
    public static void smartCopy(String fm1, String fromFile, String fm2, String toFile)
    {
        ...
        if (directory.exists() && directory.isDirectory())
        {
            ...
            try
            {
                ...
                ObjectInputStream objectInputStream = new ObjectInputStream(new BufferedInputStream(new FileInputStream(file1)));
                FileImpl oldFile = (FileImpl) objectInputStream.readObject();
                oldFile.setFileManager(fm2);
                oldFile.setFilename(toFile);
    ...
    private static int getWhereNum(String where)
    {
        switch (where)
        {
            ...
            case "t":
                return 2;
            default:
                throw new ErrorCodeException(ErrorCodeException.WRONG_WHERE);
        }
    }

    private static void writeBM(int time)
    {
