...
public class FileImpl implements File, Serializable
{
    ...
    public FileImpl(String fileManager, String fileId, int fileSize)
    {
        this.fileId = fileId;
    ...
    public void write(byte[] content)
    {
        ...
        java.io.File file = new java.io.File("src/" + this.fileManager + "/" + this.fileId + ".meta");
    ...
    public long move(long offset, int where)
    {
        switch (where){
            case MOVE_CURR:
                if (cursor+offset>fileSize-1 || cursor+offset<0){
            ...
            case MOVE_TAIL:
                ...
                cursor= fileSize-offset-1;
                System.out.println("当前光标为"+cursor);
                break;
            default:System.out.println("where值错误");
        }
        return cursor;
    }
    ...
    public void setSize(long newSize)
    {
        ...
        else if (newSize > fileSize)
        {
            ...
            if(newSize>(blockLists.length/2)*blockSize){
                ...
                String[][] newBlockInfo=
                        blockManager.allocateBlock(newContent);
    ...
    public void updateFileMeta(){
        java.io.File file=new java.io.File("src/"+fileManager+"/"+fileId+".meta");
        try
        {
